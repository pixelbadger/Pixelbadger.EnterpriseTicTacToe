...
public sealed record RequestRematchCommand(string SessionCode, string ClientIdentity) : ICommand<Result<GameStateDto>>;
...
public sealed class RequestRematchCommandHandler(
    ...
    : ICommandHandler<RequestRematchCommand, Result<GameStateDto>>
{
    public async ValueTask<Result<GameStateDto>> Handle(RequestRematchCommand command, CancellationToken cancellationToken)
    {
